...
const Notes = ({ note, bouteilleId }) => {
    ...
    const onSelectChange = (e, bouteilleId) => {
        ...
        setOpacityState("0");
        setNoteBouteille(parseInt(e.target.value))
    };
    ...
    return (
        <NotesCountainer>
            <div>
                <span>
                    {[...Array(noteBouteille || note)].map((e, i) =><img key={i} src={Tache} alt="tache de vin "/>)}
                </span>
                <span>
            ...
            <div>
                    ...
                    <Select style={{opacity: `${setOpacity}`, transition: "opacity 0.4s ease"}} name="cellier" onChange={(e)=>onSelectChange(e, bouteilleId)} value={noteBouteille || note}>
